package xyz.base
{
	 ...
	 */
	public final class TooQuaternion
	{
		/**
		 ...
		 */
		public function TooQuaternion(x : Number = 0, y : Number = 0, z : Number = 0, w : Number = 1)
		{
			this.x = x;
			this.y = y;
			this.z = z;
		 ...
		 */
		public function multiply(qa:TooQuaternion, qb:TooQuaternion):void
		{
		...
		public function multiplyVector(vector : Vector3D, target : TooQuaternion = null) : TooQuaternion
		{
			target ||= new TooQuaternion();
		 ...
		 */
		public function slerp(qa : TooQuaternion, qb : TooQuaternion, t : Number) : void
		{
			var w1 : Number = qa.w, x1 : Number = qa.x, y1 : Number = qa.y, z1 : Number = qa.z;
			var w2 : Number = qb.w, x2 : Number = qb.x, y2 : Number = qb.y, z2 : Number = qb.z;
		 ...
		 * @param t The interpolation weight, a value between 0 and 1.
		 */
		public function lerp(qa : TooQuaternion, qb : TooQuaternion, t : Number) : void
		{
			var w1 : Number = qa.w, x1 : Number = qa.x, y1 : Number = qa.y, z1 : Number = qa.z;
			var w2 : Number = qb.w, x2 : Number = qb.x, y2 : Number = qb.y, z2 : Number = qb.z;
			var len : Number;
		...
		{
			var rawData : Vector.<Number> = TooMatrix3DUtils.RAW_DATA_CONTAINER;
			...
			rawData[0] = xx - yy - zz + ww;		rawData[4] = xy2 - zw2;					rawData[8] = xz2 + yw2;   				rawData[12] = 0;
			rawData[1] = xy2 + zw2;				rawData[5] = -xx + yy - zz + ww;		rawData[9] = yz2 - xw2;					rawData[13] = 0;
			rawData[2] = xz2 - yw2;				rawData[6] = yz2 + xw2;					rawData[10] = -xx - yy + zz + ww;		rawData[14] = 0;
			rawData[3] = 0.0;					rawData[7] = 0.0;						rawData[11] = 0;						rawData[15] = 1;
		...
		/**
		 * Converts the quaternion to a Vector. matrix representation of a rotation equivalent to this quaternion.
		 * @param target The Vector. to contain the raw matrix data.
		...
		public function toRawData(target : Vector.<Number>, exclude4thRow : Boolean = false) : void
		{
			...
			target[0] = xx - yy - zz + ww;		target[1] = xy2 - zw2;					target[2] = xz2 + yw2;
			target[4] = xy2 + zw2;				target[5] = -xx + yy - zz + ww;			target[6] = yz2 - xw2;
			target[8] = xz2 - yw2;				target[9] = yz2 + xw2;					target[10] = -xx - yy + zz + ww;
		 ...
		 */
		public function clone() : TooQuaternion
		{
			return new TooQuaternion(x, y, z, w);
		}
		 ...
		 */
		public function copyFrom(q : TooQuaternion) : void
		{
			x = q.x;
			y = q.y;
			z = q.z;
		...
		public function traceMathSense():void{
//			x = s * Xa
//			y = s * Xb
//			z = s * Xc
//			w = cos(θ/2)
//			s = sin(θ/2)
			...
			var xb:Number = y/s;
			var xc:Number = z/s;
			var ang:Number = 2*angle/Math.PI*180;
			var v3d:Vector3D = new Vector3D(xa,xb,xc);
			//v3d.normalize()
			trace("绕轴<" + v3d + ">转动" + ang + "度");
		}
		
		public function rotationX():void{
			var angle:Number = Math.acos(w);
			var s:Number = Math.sin(angle);
			var xa:Number = x/s;
			var xb:Number = y/s;
			...
			v3d = utilsMatrix.transformVector(v3d);
			x = s*v3d.x;
			y = s*v3d.y;
			z = s*v3d.z;
		}
