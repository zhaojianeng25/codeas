package com.zcp.loader
{
	 ...
	 * 
	 * 此类功能：
	 * 1.加载Loader可加载的显示对象(SWF\PNG\等),此时必须为LoadeData传入container参数;此文件可谓加密压缩格式，但需要为LoadeData传入decode参数.
	 * 2.加载SWF内的导出类到应用程序当前域,此时必须为LoadeData传入null的container参数;此文件可谓加密压缩格式，但需要为LoadeData传入decode参数.
	 * 
	 * 回调函数格式：function callBack(LoadData, Event);void{}
	 * 
	 * 加载之后会自动执行LoadData中的解压解密函数
	 * 并没有做同一加载地址的处理
	 * 
	 * @author zcp
	 * 
	...
	public class DobjLoadManager
	{
		 ...
		 * data: RslLoader
		 * */
		private static var loaderList:Vector.<DobjLoader> = new Vector.<DobjLoader>();
		
		
		/**
		 * 并行加载的默认最大上限
		 */		
		private static const MAX_THREAD_NUM:uint = 20;
		/**并行加载最大条数*/
		private static var _max_thread:uint = MAX_THREAD_NUM;
		...
		public static function load($ld:LoadData):void
		{
			...
			//如果当前加载组正在加载则返回
			if(ldGroup.dobjLoader!=null)return;
			
			
			//取得一个空闲的加载器,如果加载器不为空,则开始执行加载
			var rslLoader:DobjLoader = getFreeLoader(ldGroup.priority);
			if(rslLoader!=null)
			{
				//开始执行加载
				loadNext(rslLoader);
		 ...
		 * @param $decode	解密函数
		 * @param $container 存入容器
		 * 
		 */		
		public static function lazyLoad($url:String, $dataFormat:String=null,$callBack:Function=null, $priority:int=0, $decode:Function=null,$container:DisplayObjectContainer=null):void
		{
			//生成简单的LoadData
			var loadData:LoadData = new LoadData($url,$callBack,null,null,"","",$priority,$dataFormat,$decode,$container);
		...
		public static function cancelLoadByUrl($url:String):void
		{
			...
			for(var i:int=len-1; i>=0; i--)
			{
				...
				if(ldg.url==$url)
				{
					...
					//停止加载
					var dl:DobjLoader = ldg.dobjLoader;
					if(dl!=null)
					{
						removeLoadEvent(dl);
						dl.stop();
		...
		public static function set maxThread($value:uint):void
		{
			...
			else//否则暂停一些加载
			{
				...
				if(len>0)
				{
					...
					{
						var dl:DobjLoader = loaderList[i];
		 ...
		 *  
		 */			
		private static function loadNext($rslLoader:DobjLoader=null):void
		{
			var rslLoader:DobjLoader = $rslLoader;
		...
		private static function rslLoaderHandler(e:Event) : void
		{
			...
			switch(e.type)
			{
				...
				case SecurityErrorEvent.SECURITY_ERROR:
					ZLog.add("DobjLoaderManager: 尝试"+rslLoader.loadDataGroup.headLoadData.tryCount+"次后，加载"+rslLoader.loadDataGroup.url+"失败");
