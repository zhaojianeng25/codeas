package renderLevel.backGround
{
	...
	public class BackGroundDisplay3DSprite extends Display3DSprite
	{
		...
		public function setInfoDataAtf(zNum:Number,url:String):void
		{
			_objData=MakeModelData.makeJuXinTampData(new Vector3D(0,0,zNum),new Vector3D(2,-2,zNum));
			LoadManager.getInstance().addSingleLoad( new LoadInfo(url, LoadInfo.BYTE, function onBitmapLoad(bitmap:ByteArray):void
			{
				_objData.texture=TextureManager.getInstance().atfToTexture(bitmap);
				_width=512
				_height=512
				uplodToGpu();
			}, 0));	
