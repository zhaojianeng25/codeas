		...
		private function querySuc(rs:ResultSet):void{
			...
			if(rs.next()){
				...
				if(getToolPermission(obj.tool)){
					...
					AppDataBone.projectName = String(_configXML.projectName[obj.project].@name);
					AppDataBone.statusString = "user:" + obj.username + "---《" + AppDataBone.projectName + "》";
					AppDataBone.isAuthorize = true;
					AppDataBone.mapUrl = dbObj.mapUrl;
					AppDataBone.projectType = obj.project;
				}else{
					AppDataBone.statusString = "当前账户没有操作此编辑器的权限!";
				}
				
			}else{
				DBManager.getInstance().disConnect();//断开数据库连接
				AppDataBone.statusString = "保存的账号不存在或者密码不正确！";
			}
		}
