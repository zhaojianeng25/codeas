package view.byteFile
{
	...
	public class AnimFileToByteUtils
	{
		...
		private function writeBindPos():void{
			
			var boneAry:Vector.<ObjectBone> = MeshPanel.getInstance().getMeshData().boneItem;
			
			boneAry = processBoneNew(boneAry);
			
			byte.writeInt(1);
			
			byte.writeInt(boneAry.length);
			for(var i:int;i<boneAry.length;i++){
				byte.writeInt(boneAry[i].father);
				
				byte.writeFloat(boneAry[i].tx);
				byte.writeFloat(boneAry[i].ty);
				byte.writeFloat(boneAry[i].tz);
				
				byte.writeFloat(boneAry[i].qx);
				byte.writeFloat(boneAry[i].qy);
				byte.writeFloat(boneAry[i].qz);
			}
		}
