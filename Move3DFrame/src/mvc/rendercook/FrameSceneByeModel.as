package mvc.rendercook
{
	...
	public class FrameSceneByeModel
	{
		public function FrameSceneByeModel()
		{
		}
		public static function getInstance():FrameSceneByeModel{
			if(!instance){
				instance = new FrameSceneByeModel();
			}
			return instance;
		}
		...
		public function resetTexture($uid:String,$bmp:BitmapData):void
		{
			...
			var $item:Vector.<FrameFileNode>=new Vector.<FrameFileNode>
			for(var i:uint=0;i<_listArr.length;i++){
				var $FrameFileNode:FrameFileNode=_listArr[i] as FrameFileNode ;
				if($FrameFileNode.url.search(".prefab")!=-1){
					if($FrameFileNode.iModel&&$FrameFileNode.iModel.uid==$uid)
					{
						var $proxyPan3dModel:Object=$FrameFileNode.iModel
		 ...
		 * 
		 */
		private function setRenderItem($arr:Vector.<FileNode>):void
		{
		...
		private function makeMaterialBaseData($arr:Vector.<FileNode>):void
		{
			...
			for(var i:uint=0;i<$arr.length;i++){
				...
				if($FrameFileNode.url.search(".prefab")!=-1){
				//if($FrameFileNode.type==HierarchyNodeType.Prefab){
		...
		{
			var _lightItem:Vector.<FrameFileNode>=new Vector.<FrameFileNode>
			for(var i:uint=0;i<$arr.length;i++){
				var $FrameFileNode:FrameFileNode=$arr[i] as FrameFileNode ;
				if($FrameFileNode.type==HierarchyNodeType.Light){
					_lightItem.push($FrameFileNode)
				}
			}
		...
		{
			var paralleLightItem:Vector.<FrameFileNode>=new Vector.<FrameFileNode>
			for(var u:uint=0;u<$arr.length;u++){
				var $FrameFileNode:FrameFileNode=$arr[u] as FrameFileNode ;
				if($FrameFileNode.type==HierarchyNodeType.ParallelLight){
					paralleLightItem.push($FrameFileNode)
				}
			}
			var $byte:ByteArray=new ByteArray
			if(this.useLittleEncode){
		...
		{
			var $FrameFileNode:FrameFileNode
			var $item:Vector.<FrameFileNode>=new Vector.<FrameFileNode>
			...
			var $idItem:Vector.<FrameFileNode>=new Vector.<FrameFileNode>
			for(var u:uint=0;u<$arr.length;u++){
				$FrameFileNode=$arr[u] as FrameFileNode ;
				if($FrameFileNode.type==HierarchyNodeType.LightProbe){
					$probeMeshItem.push(LightProbeStaticMesh($FrameFileNode.data))
					$idItem.push($FrameFileNode)
				}
			}
			var $byte:ByteArray=new ByteArray
			if(this.useLittleEncode){
		...
		private function makeObjData($arr:Vector.<FileNode>):void
		{
			...
			for(var i:uint=0;i<$arr.length;i++){
				$FrameFileNode=$arr[i] as FrameFileNode ;
				if($FrameFileNode.url.search(".prefab")!=-1){
				
					//trace("是否渲染",$buildMesh.isNotCook)
					var $needCook:Boolean;
					if(ProxyPan3dModel($FrameFileNode.iModel)&&ProxyPan3dModel($FrameFileNode.iModel).sprite&&ProxyPan3dModel($FrameFileNode.iModel).sprite.material){
						$needCook=!ProxyPan3dModel($FrameFileNode.iModel).sprite.material.noLight
					}else{
						$needCook=false
						
					//	Alert.show("渲染有问题:"+$FrameFileNode.name,"对象有错")
					}
					trace("是否渲染",$needCook)
					if($needCook){
						$item.push($FrameFileNode)
					}
			...
			for(var j:uint=0;j<$item.length;j++){
				$FrameFileNode=	$item[j]
		
				var $imode:IModel=$FrameFileNode.iModel
				var $objectData:ObjData=ProxyPan3dModel($imode).sprite.objData
				
				if(!$objectData){   //当没有模型数据信息时
					Alert.show($FrameFileNode.name+"没有模型")
					stopRender()
					return 
				}
				$byte.writeInt($FrameFileNode.type)   //类形
				$byte.writeInt($FrameFileNode.id)    //id
				$byte.writeUTF(String($FrameFileNode.id))
				
				
				
				var $proxyPan3dModel:ProxyPan3dModel=$FrameFileNode.iModel as ProxyPan3dModel
				var $sprite:Display3DMaterialSprite=$proxyPan3dModel.sprite  as Display3DMaterialSprite
				...
				if($FrameFileNode.lightuvSize){
					$byte.writeInt($FrameFileNode.lightuvSize)
				}else{
					$byte.writeInt(128)//	$byte.writeInt($buildMesh.lightMapSize)
				}
				$byte.writeInt(3) //	$byte.writeInt($buildMesh.lightBlur)
				var _prefab:PrefabStaticMesh=PrefabManager.getInstance().getPrefabByUrl(AppData.workSpaceUrl+ $FrameFileNode.url);
					
				$byte.writeUTF(_prefab.axoFileName)  //模型地址
				...
				$byte.writeBoolean(false)//		$byte.writeBoolean($buildMesh.lightProbe)//lightProbe
		...
		private function writerGroundObj($byte:ByteArray):void
		{
			for (var i:uint=0;i< GroundManager.getInstance().groundItem.length;i++)
			{
				...
				}else
				{
					...
					//					$byte.writeInt($FrameFileNode.type)   //类形
					//					$byte.writeInt($FrameFileNode.id)    //id
					//					$byte.writeUTF("build"+String($FrameFileNode.id))
		...
		private static var instance:FrameSceneByeModel;
		/**
		 *生存地面贴图图片。并记录下图片地地址 
		 * 
		 */
