package   scenelevel
{
	...
	public class FrameKeyControl extends EventDispatcher
	{
		private static var instance:FrameKeyControl;
		
		public static function getInstance():FrameKeyControl
		{
			if(!instance)
			{
				instance = new FrameKeyControl();
			}
			return instance;
		}
				...
				if(_isMiddleDown)
				{
					...
					if(event.altKey){
						...
						if(middleMovetType==false){
							...
							var $hy:Number=Math.max(1,Math.abs(Scene_data.cam3D.y/500)) //移动参数
							Scene_data.cam3D.x=_mouseInfo._last_rx+	(_middleMoveVe.x-$v.x)*$hy;
							Scene_data.cam3D.y=_mouseInfo._last_ry+	(_middleMoveVe.y-$v.y)*$hy;
							Scene_data.cam3D.z=_mouseInfo._last_rz+	(_middleMoveVe.z-$v.z)*$hy;
		...
		protected function onKeyDown(event:KeyboardEvent):void
		{
			...
			if(event.target  as  RichEditableText||event.target  is  RichEditableText)
			{
				return;
			}
