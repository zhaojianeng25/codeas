...
class ArtFont {
    ...
    //将美术字写到ctx上 左对齐的
    public writeFontToCtxLeft($ctx: CanvasRenderingContext2D, $str: string, $color: string = ArtFont.num1, $tx: number = 0, $ty: number = 0, $txtInterval: number = 0): void {
    ...
    public writeFontToSkinName($UIAtlas: UIAtlas, $iconName: string, $str: string, $color: string = ArtFont.ORANGE_TXT, $textAlign: string = TextAlign.LEFT, $txtInterval: number = 0): number {
        ...
        if ($textAlign == TextAlign.LEFT) {
            this.writeFontToCtxLeft($UIAtlas.ctx, $str, $color, 0, 0, $txtInterval)
        }
