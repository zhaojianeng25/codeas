class Display3dMovie extends Display3DSprite implements IBind{
    ...
    protected _curentAction: string;
    ...
    */
    ...
    public getSocket(socketName: string, resultMatrix: Matrix3D): void {
        ...
        if (!this._skinMesh || !this._skinMesh.boneSocketDic[socketName]){
    ...
    protected getFrameMatrix(index: number): Matrix3D {
        ...
        if (this._animDic[this._curentAction]) {
            var animData: AnimData = this._animDic[this._curentAction];
    ...
    public addAction(name: string, url: string, needPerLoad: boolean = false): void {
        ...
        if (name == this._defaultAction || name == this._curentAction) {
    ...
    public play($action: string, $completeState: number = 0,needFollow:boolean = true): boolean {
        ...
        if (this._curentAction == $action) {
            return;
        }
        //FpsMc.tipStr = "2";
        this._curentAction = $action;
    ...
    public updateFrame(t: number): void {
        ...
        if (this._curentAction && this._animDic[this._curentAction]) {
            actionKey = this._curentAction;
        } else if (this._animDic[this._defaultAction]) {
        ...
        if (this._curentFrame >= animData.matrixAry.length) {
            if (this._completeState == 0) {
                ...
                this.changeAction(this._curentAction)
    ...
    {
        this._curentAction = this._defaultAction;
    }
    ...
    public setMeshVc($mesh: MeshData): void {
        ...
        if (this._animDic[this._curentAction]) {
             animData = this._animDic[this._curentAction];
        } else if (this._animDic[this._defaultAction]){
