var AstarUtil = (function () {
    ...
    AstarUtil.sceneRotationInfo = function () {
        if (!this.navmeshData) {
            return;
        }
        if (this._sceneVectList) {
            var $focus2D = AstarUtil.getGrapIndexByPos(new Vector3D(Scene_data.focus3D.x, Scene_data.focus3D.y, Scene_data.focus3D.z));
            for (var i = 0; i < this._sceneVectList.length; i++) {
                var $pos = new Vector2D(this._sceneVectList[i].x, this._sceneVectList[i].y);
                var $dis = Vector2D.distance($pos, $focus2D);
                this._sceneVectList[i].z = $dis;
            }
            this._sceneVectList.sort(function (a, b) {
                return a.z - b.z;
            });
            var disA = Vector2D.distance(new Vector2D(this._sceneVectList[0].x, this._sceneVectList[0].y), $focus2D);
            var disB = Vector2D.distance(new Vector2D(this._sceneVectList[1].x, this._sceneVectList[1].y), $focus2D);
            var $kangly = disA / (disA + disB) * this._sceneVectList[1].w + disB / (disA + disB) * this._sceneVectList[0].w;
            if (this._frist) {
                this._frist = false;
