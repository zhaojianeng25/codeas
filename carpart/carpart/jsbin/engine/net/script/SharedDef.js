...
var SharedDef = (function () {
    ...
    //  家族地图信息
    SharedDef.FACTION_MAP_ID = 5001;
    SharedDef.FACTION_FUHUO_X = 157;
    SharedDef.FACTION_FUHUO_Y = 84;
    ...
    SharedDef.BORN_X = 138; // 出生坐标x 
    SharedDef.BORN_Y = 174; // 出生坐标y
    ...
    // 购买
    SharedDef.LOG_ITEM_OPER_TYPE_SHOP_BUY = 101; // 从商场购买获得
    ...
    SharedDef.MAX_MONEY_CHANGE_TYPE = 103;
    // 特殊包裹类型
    ...
    // 货币类型
    SharedDef.MONEY_TYPE_GOLD_INGOT = 0; // 元宝
    SharedDef.MONEY_TYPE_BIND_GOLD = 1; // 绑定元宝
    SharedDef.MONEY_TYPE_SILVER = 2; // 身上的银子
    SharedDef.MONEY_TYPE_SILVER_WAREHOUSE = 3; // 仓库的银子
    SharedDef.MONEY_TYPE_GOLD_WAREHOUSE = 4; // 仓库元宝
    SharedDef.MONEY_TYPE_BIND_GOLD_WAREHOUSE = 5; // 仓库的绑元
    SharedDef.MONEY_TYPE_QI = 6; // 真气
    SharedDef.MONEY_TYPE_BEAST = 7; // 兽灵
    SharedDef.MONEY_TYPE_GEM = 8; // 宝石精华
    SharedDef.MONEY_TYPE_FACTION = 9; // 帮派贡献
    SharedDef.MONEY_TYPE_HONOR = 10; // 荣誉
    SharedDef.MONEY_TYPE_DOUHUN = 11; // 斗魂
    ...
    SharedDef.PLAYER_PASSIVE_SPELL_MAX_COUNT = 50; //  被动技能个数
    // 玩家能否做某事的flag偏移定义
    ...
    SharedDef.UINT_FIELD_BOOS_CUR_SPELLID = 131; // 当前正在释放的蓄力技能id
    ...
    SharedDef.UINT_INT_FIELD_PASSIVE_SPELL_END = 184; // 被动技能结束
    SharedDef.MAX_UNIT_FIELD = 185;
    ...
    SharedDef.MAX_UNIT_STRING_FIELD = 7;
    // 游戏对象类型
    ...
    SharedDef.PLAYER_EXPAND_INT_MAP_STATUS = 435; // 地图开启进度
    SharedDef.PLAYER_EXPAND_INT_GAG_END_TIME = 436; // 禁言结束时间
    SharedDef.PLAYER_EXPAND_INT_TO_MAP = 437; // 玩家要传送到的地图
    SharedDef.PLAYER_EXPAND_INT_TO_INSTANCE_ID = 438; // 玩家要传送到的地图实例ID
    SharedDef.PLAYER_EXPAND_INT_TO_POS_X = 439; // 玩家药传送到的坐标
    SharedDef.PLAYER_EXPAND_INT_TO_POS_Y = 440;
    SharedDef.PLAYER_EXPAND_INT_TO_LINE_NO = 441; // 传送线号
    SharedDef.PLAYER_EXPAND_INT_DB_MAP = 442; // 玩家最后一次进入副本前的地图	
    SharedDef.PLAYER_EXPAND_INT_DB_POS_X = 443; // 玩家最后一次进入副本前的坐标
    SharedDef.PLAYER_EXPAND_INT_DB_POS_Y = 444;
    SharedDef.PLAYER_EXPAND_INT_CLIENT_DATA = 445; // 远程存储
    // ////////////////////////////////////////////////////////////////////////
    // 应用服部分	
    SharedDef.PLAYER_APPD_INT_FIELD_FLAG = 545; // 玩家标志位
    SharedDef.PLAYER_APPD_INT_FIELD_FLAG_END = 552; // 结束下标
    SharedDef.PLAYER_APPD_INT_FIELD_DAILY_TIME = 552; // 每天在线时长（min）
    SharedDef.PLAYER_APPD_INT_FIELD_RESET_DALIY = 553; // 重置的时间点每小时一次
    SharedDef.PLAYER_APPD_INT_FIELD_RECHARGE_SUM = 577; // 累计充值总数
    SharedDef.PLAYER_APPD_INT_FIELD_LAST_RECHARGE_TIME = 578; // 最后一笔充值时间
    SharedDef.PLAYER_APPD_INT_FLELD_JHM_FLAGS = 579; // 激活码flags
    SharedDef.PLAYER_APPD_INT_FLELD_JHM_FLAGS_END = 587; // 激活码flags_end
    // 主背包扩展时间戳
    SharedDef.PLAYER_APPD_INT_FIELD_EXTENSION_END_TIME = 588; // 主包裹扩展结束时间戳
    SharedDef.PLAYER_APPD_INT_FIELD_EXTENSION_REMAIN_TIME = 589; // 主包裹扩展剩余多少时间
    SharedDef.PLAYER_INT_FIELD_BLOCK_COUNT = 591; // 屏蔽列表个数
    SharedDef.PLAYER_INT_FIELD_WORLD_BOSS_JOIN_ID = 592; // 最后一次参加的世界BOSS id
    SharedDef.PLAYER_INT_FIELD_WORLD_BOSS_JOIN_STATE = 593; // 0:最后一次参加的世界BOSS状态, 1:进入的线数,2:死亡次数,3:预留
    // 场景服扩展下标开始了
    SharedDef.PLAYER_SCENED_INT_FLAGS = 594; // 玩家场景服标志位
    //  只显示装备的主动技能
    SharedDef.PLAYER_INT_FIELD_SPELL_START = 602;
    SharedDef.PLAYER_INT_FIELD_SPELL_END = 614;
    // 重要技能的CD
    SharedDef.PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_START = 614;
    SharedDef.PLAYER_INT_FIELD_IMPORTANT_SPELL_CD_END = 654;
    SharedDef.PLAYER_EXPAND_KILL_MONSTER = 654; // 杀怪数
    // 跨服
    SharedDef.PLAYER_APPD_INT_FIELD_KUAFU_WARID = 655; // 跨服场次id
    SharedDef.PLAYER_INT_FIELD_KUAFU_NUMBER = 656; // 0:玩家报名跨服时收到匹配信息中给的编号 1:跨服类型（值参照枚举：EKUAFU_TYPE）
    SharedDef.PLAYER_INT_FIELD_MAIN_QUEST_ID = 657; // 当前主线任务
    SharedDef.PLAYER_INT_FIELD_QUEST_CHAPTER = 658; // 任务章节奖励领取情况
    SharedDef.PLAYER_INT_FIELD_MOUNT_LEVEL = 659; // 4个bytes(0:当前坐骑阶数 1:当前坐骑星级,2:当前坐骑是否骑乘，3:当前幻化id)
    SharedDef.PLAYER_INT_FIELD_DIVINE_ID = 660; // 当前装备的神兵id
    SharedDef.PLAYER_INT_FIELD_PASSIVE_SPELL_START = 661; // 被动技能开始
    SharedDef.PLAYER_INT_FIELD_PASSIVE_SPELL_END = 711; // 被动技能个数
    SharedDef.PLAYER_INT_FIELD_VIRTUAL_CAMP = 711; // 虚拟阵营
    SharedDef.PLAYER_INT_FIELD_WORLD_3V3_SCORE = 712; // 跨服3v3积分
    SharedDef.PLAYER_INT_FIELD_WORLD_3V3_TOTAL_SCORE = 713; // 跨服3v3积分
    SharedDef.PLAYER_INT_FIELD_WORLD_3V3_TREND_INFO = 714; // 胜负走势
    SharedDef.PLAYER_INT_FIELD_DOUJIANTAI_RANK = 715; // 斗剑台排名
    SharedDef.PLAYER_INT_FIELD_DOUJIANTAI_REFRESH_TIME = 716; // 最后一次刷新对手时间戳
    SharedDef.PLAYER_INT_FIELD_ARREARS = 717; // 欠费
    SharedDef.PLAYER_INT_FIELD_DAILY_QUEST_FINISHED = 719; //  每日任务完成数
    SharedDef.PLAYER_INT_FIELD_BAG_SORT_TIME = 720; // 整理背包时间戳
    // 已开启功能的记录
    SharedDef.PLAYER_INT_FIELD_OPEN_MENU_INDEX = 721;
    SharedDef.PLAYER_INT_FIELD_OPEN_MENU_START = 722;
    SharedDef.PLAYER_INT_FIELD_OPEN_MENU_END = 822;
    // 已购家族商店物品的记录
    SharedDef.PLAYER_INT_FIELD_BUYED_FACTION_SHOP = 823; // 已购家族商店物品开始
    SharedDef.PLAYER_INT_FIELD_BUYED_FACTION_SHOP_END = 831; // 已购家族商店物品结束
    // 记录强制引导ID 
    SharedDef.PLAYER_INT_FIELD_GUIDE_ID_LAST = 832; // 上次完成的引导
    SharedDef.PLAYER_INT_FIELD_GUIDE_ID_NOW = 833; // 当前进行中的引导
    // 修炼场相关
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LAST_PLUNDER_TIME = 834; // 上次挑战时间戳
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LAST_REFRESH_TIME = 835; // 上次刷新对手时间戳
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_EXP = 836; // 当日累计掠夺经验
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_PLUNDER_CHEST = 837; // 当日累计掠夺宝箱数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_PURCHASE_COUNT = 838; // 当日购买次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LEFT_PLUNDER_COUNT = 839; // 当前剩余挑战次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_PLUNDER_RECORD_INDEX = 840; // 修练场修炼记录下标
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TOTAL_BEEN_PLUNDER_COUNT = 841; // 本轮被掠夺成功次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_TODAY_TOTAL_PLUNDER_COUNT = 842; // 本日总掠夺次数
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_START_TIME = 843; // 修炼开始时间
    SharedDef.PLAYER_INT_FIELD_CULTIVATION_LOST_INFO = 844; // 修炼宝箱丢失信息
    // 登陆大礼相关
    SharedDef.PLAYER_INT_FIELD_LOGIN_ACTIVITY_TOTAL_DAYS = 845; // 登陆总天数
    SharedDef.PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_START = 846; // 领取状态开始
    SharedDef.PLAYER_INT_FIELD_LOGIN_ACTIVITY_REWARD_STATUS_END = 848; // 领取状态结束
    //  非强制引导ID
    SharedDef.PLAYER_INT_FIELD_OPTIONAL_GUIDE_START = 849; // 非强制引导开始
    SharedDef.PLAYER_INT_FIELD_OPTIONAL_GUIDE_END = 909; //  非强制引导结束
    SharedDef.PLAYER_INT_FILED_LEAVE_RISK_TIME = 909; // 离开冒险世界的时间戳
    SharedDef.PLAYER_INT_FILED_LEAVE_RISK_PICKED = 910; // 是否已经领取不在冒险世界的挂机奖励
    SharedDef.PLAYER_INT_FILED_FACTION_BUILDING_SPEEDUP_DAILY_COUNT = 911; // 家族建筑升级加速每日次数
    SharedDef.PLAYER_INT_FILED_FACTION_DONATE_GIFT_EXCHANGE_DAILY_COUNT = 912; // 家族宝库贡献礼物兑换每日次数
    // ////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 字符串部分
    ...
    SharedDef.SPELL_INT_FIELD_MOUNT_SPELL_START = 33; // 进阶技能开始
    SharedDef.SPELL_INT_FIELD_MOUNT_SPELL_END = 43; // 进阶技能结束
    SharedDef.SPELL_INT_FIELD_MOUNT_BLESS_EXP = 43; // 进阶祝福值
    SharedDef.SPELL_INT_FIELD_MOUNT_ILLUSION_START = 44; // 幻化开始
    SharedDef.SPELL_INT_FIELD_MOUNT_ILLUSION_END = 164; // 幻化结束
    SharedDef.SPELL_STRENGTHEN_START = 165; // 强化开始
    SharedDef.SPELL_STRENGTHEN_END = 175; // 强化结束
    SharedDef.SPELL_STRENGTHEN_ALLMUL = 176; // 全身强化标记
    SharedDef.SPELL_GEM_START = 177; // 宝石开始
    SharedDef.SPELL_GEM_END = 217; // 宝石结束
    SharedDef.SPELL_GEM_ALLMUL = 218; // 全身宝石标记
    SharedDef.SPELL_DIVINE_COUNT = 219; // 神兵个数
    SharedDef.SPELL_DIVINE_START = 220; // 神兵开始
    SharedDef.SPELL_DIVINE_END = 310; // 神兵结束
    ...
    SharedDef.SOCIAL_FRIEND_END = 126; // 好友结束
    SharedDef.SOCIAL_ENEMY_START = 126; // 仇人开始
    SharedDef.SOCIAL_ENEMY_END = 166; // 仇人结束
    SharedDef.SOCIAL_APPLY_START = 166; // 申请开始
    SharedDef.SOCIAL_APPLY_END = 186; // 申请结束
    SharedDef.SOCIAL_APPLY_CLEAR_FLAG = 186; // 申请清除标记
    SharedDef.SOCIAL_ENEMY_TIME_START = 187; // 仇人时间戳开始
    SharedDef.SOCIAL_ENEMY_TIME_END = 207; // 仇人时间戳结束
    // ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // 帮派下标
    // 帮派事件
    ...
    SharedDef.MAX_FACTION_INT_MEMBER = 7;
    // 帮派成员的STRING信息
    ...
    SharedDef.MAX_FACTION_STOREHOUSE_RECORD_COUNT = 20;
    // 帮派下标
    // UINT32 部分
    ...
    SharedDef.FACTION_INT_FIELD_PLAYER_END = 352; // 帮派申请索引
    SharedDef.FACTION_INT_FIELD_APPLY_PLAYER_COUNT_INDEX = 352;
    SharedDef.FACTION_INT_FIELD_APPLY_PLAYER = 353; // 玩家申请加入帮派列表
    SharedDef.FACTION_INT_FIELD_APPLY_PLAYER_END = 374; // 帮派申请玩家列表
    SharedDef.FACTION_INT_FIELD_EVENT = 374; // 帮派记录开始
    SharedDef.FACTION_INT_FIELD_EVENT_END = 394; // 帮派记录结束
    SharedDef.FACTION_INT_FIELD_EVENT_FALG = 394; // 帮派记录标志位
    SharedDef.FACTION_INT_FIELD_SHOP = 395; // 商店开始
    SharedDef.FACTION_INT_FIELD_SHOP_END = 403; // 商店结束
    SharedDef.FACTION_INT_FIELD_FORCE = 403; // 帮派战斗力
    SharedDef.FACTION_INT_FIELD_MONEY = 405; // 帮派资产
    SharedDef.FACTION_INT_FIELD_LEVEL = 407; // 帮派等级
    SharedDef.FACTION_INT_FIELD_ACTIVITY = 408; // 帮派活跃度
    SharedDef.FACTION_INT_FIELD_RANK = 409; // 帮派排名
    SharedDef.FACTION_INT_FIELD_FLAGS_ID = 410; // (byte)0:帮派当前选择的旗帜ID (byte)1:神明等级 (uint16)1:灵气
    SharedDef.FACTION_INT_FIELD_MINLEV = 411; // 加入帮派最小等级
    SharedDef.FACTION_INT_FIELD_TOKEN_NUM = 412; // 令牌数量
    SharedDef.FACTION_INT_FIELD_TOKEN_POINTS = 413; // 令牌今日积分进度
    SharedDef.FACTION_INT_FIELD_TOKEN_POINTS_COUNT = 414; // 令牌今日积分完成次数
    SharedDef.FACTION_INT_FIELD_TOKEN_TODAY_BUY_COUNT = 415; // 今日令牌购买次数
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_ID_MAX = 416; // 已挑战最大boss id
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_ID = 417; // 当前挑战boss id
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_START_TIME = 418; // 当前挑战开始时间
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_HP_RATE = 419; // 当前boss血量万分比
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_POS = 420; // 当前boss位置 x,y
    SharedDef.FACTION_INT_FIELD_PROTECT_TARGET_HP_RATE = 421; // 当前守护目标血量万分比
    SharedDef.FACTION_INT_FIELD_PROTECT_TARGET_POS = 422; // 当前守护目标位置 x,y
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_DAMAGE_RANK_START = 423; // 当前boss输出榜开始 int 输出值
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_DAMAGE_RANK_END = 473; // 当前boss输出榜结束
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_TOTAL_RANK_START = 473; // 总输出榜开始
    SharedDef.FACTION_INT_FIELD_CHALLENGE_BOSS_TOTAL_RANK_END = 523; // 总输出榜结束
    SharedDef.FACTION_INT_FIELD_STOREHOUSE_ITEM_COUNT = 523; // 宝库道具数量
    SharedDef.FACTION_INT_FIELD_STOREHOUSE_RECORD_CURSOR = 524; // 宝库记录游标
    SharedDef.FACTION_INT_FIELD_BUILDING_ID_START = 525; // 已有建筑id开始
    SharedDef.FACTION_INT_FIELD_BUILDING_ID_END = 535; // 已有建筑id结束
    SharedDef.FACTION_INT_FIELD_BUILDING_LVUP_ID = 535; // 当前正在升级的建筑id
    SharedDef.FACTION_INT_FIELD_BUILDING_LVUP_FINISH_TIME = 536; // 当前升级建筑完成时间戳
    ...
    SharedDef.FACTION_STRING_FIELD_STOREHOUSE_RECORD_END = 344; // 宝库记录结束
    ...
    SharedDef.INSTANCE_INT_FIELD_RES_START = 4; // 资源副本开始 每个信息4个byte[0:挑战次数,1:是否通关,2:预留,3:预留]
    SharedDef.INSTANCE_INT_FIELD_RES_END = 9; // 资源副本结束
    SharedDef.INSTANCE_INT_FIELD_ACTIVE_START = 9; // 活动开始
    SharedDef.INSTANCE_INT_FIELD_ACTIVE_END = 59; // 活动结束
    SharedDef.INSTANCE_INT_FIELD_ACTIVE = 59; // 活动-活跃度
    SharedDef.INSTANCE_INT_FIELD_ACTIVE_REWARD = 60; // 奖励领取状态
    SharedDef.INSTANCE_INT_FIELD_3V3_TIMES = 61; // 3v3次数 已参加次数 购买次数
    SharedDef.INSTANCE_INT_FIELD_3V3_DAY_REWARD = 62; // 3v3每日次数奖励 4个byte
    SharedDef.INSTANCE_INT_FIELD_3V3_SEGMENT_TIME = 63; // 3v3段位 发放时间
    SharedDef.INSTANCE_INT_FIELD_XIANFU_DAY_TIMES = 64; // 每日挑战次数
    SharedDef.INSTANCE_INT_FIELD_DOUJIANTAI_CURSOR = 65; // 斗剑台信息游标
    SharedDef.INSTANCE_INT_FIELD_DOUJIANTAI_ENEMY_S = 66; // 对手名次开始
    SharedDef.INSTANCE_INT_FIELD_DOUJIANTAI_ENEMY_E = 69; // 对手名次结束
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_TIMES = 70; // 斗剑次数 已挑战次数 购买次数
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_FIGHT_CD = 71; // 斗剑台挑战CD时间戳
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_FIRST_GET = 72; // 首胜奖励标记
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_FIRST_REWARD = 73; // 首胜奖励领取标记
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_COMBATWIN = 74; // 连胜纪录 纪录 当前
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_COMBATWIN_REWARD = 75; // 连胜奖励记录
    SharedDef.INSTANCE_INT_FIELD_DOUJIAN_MAX_RANK = 76; // 历史最高纪录
    ...
    SharedDef.ALL_MODE = 2; // 全体模式
    SharedDef.WICKED_MODE = 3; // 恶人模式
    SharedDef.SELF_DEFENSE_MODE = 4; // 自卫模式
    SharedDef.MAX_BATTLE_MODE = 5;
    // 需要发内部消息同步到场景服的技能类型
    ...
    SharedDef.GLOBALVALUE_INT_FIELD_WORLD_BOSS_TIMES = 285; // 世界BOSS次数
    SharedDef.GLOBALVALUE_INT_FIELD_WORLD_BOSS_ID = 286; // 0:世界BOSS类型1, 1:世界BOSS类型2,2:当前选定的类型,3:房间个数
    SharedDef.GLOBALVALUE_INT_FIELD_XIANFU_RECORD_CURSOR = 287; // 仙府开启宝箱的记录游标
    ...
    SharedDef.MAP_INT_FIELD_RESERVE3 = 8; // 保留字段3
    SharedDef.MAP_INT_FIELD_RESERVE4 = 9; // 保留字段4
    SharedDef.MAP_INT_FIELD_QUESTS_START = 10; // 副本任务开始
    SharedDef.MAP_INT_FIELD_QUESTS_END = 34; // 副本任务结束
    SharedDef.MAP_INT_FIELD_QUESTS_PROCESS_START = 34; // 进度开始
    SharedDef.MAP_INT_FIELD_QUESTS_PROCESS_END = 37; // 进度结束
    SharedDef.MAP_INT_FIELD_CREATE_TM = 37; // 副本创建时间
    SharedDef.MAP_INT_FIELD_START_TM = 38; // 副本开始时间
    SharedDef.MAP_INT_FIELD_QUEST_END_TM = 39; // 副本任务结束时间
    SharedDef.MAP_INT_FIELD_END_TM = 40; // 副本结束时间
    SharedDef.MAP_INT_FIELD_INSTANCE_TYPE = 41; // 副本类型
    // 地图字符串部分
    SharedDef.MAP_STR_GENERAL_ID = 4;
    SharedDef.MAP_STR_REWARD = 5; // 副本奖励物品字符串
    // 小怪陆续刷
    SharedDef.REFRESH_MONSTER_FIELD_ID = 45; // 2个short(0:当前已经刷的,1:总共需要刷多少怪)
    SharedDef.REFRESH_MONSTER_FIELD_INFO_START = 46; // 怪物信息开始2个short(0:entry,1:level)
    ...
    //  跨服3v3匹配
    SharedDef.KUAFU_3V3_FIELDS_INT_INFO_START = 42; //  跨服数据开始
    SharedDef.KUAFU_3V3_FIELDS_INT_INFO_END = 72; //  3v3总共6个人
    ...
    //  跨服仙府夺宝匹配
    SharedDef.KUAFU_XIANFU_FIELDS_INT_INFO_START = 42; //  跨服数据开始
    SharedDef.KUAFU_XIANFU_FIELDS_INT_INFO_END = 112; //  3v3总共6个人
    SharedDef.KUAFU_XIANFU_FIELDS_INT_BOSS_INDX = 112; // 当前的bossid
    SharedDef.KUAFU_XIANFU_FIELDS_INT_BOSS_INFO_START = 113; // BOSS信息开始
    SharedDef.KUAFU_XIANFU_FIELDS_INT_BOSS_INFO_END = 128;
    SharedDef.KUAFU_XIANFU_FIELDS_HARD = 128; // 副本难度
    SharedDef.XIANFU_TEST_DUMMY_COUNT = 129; // 假人数量
    SharedDef.KUAFU_XIANFU_FIELDS_STR_INFO_START = 6; //  字符串数据开始
    SharedDef.KUAFU_XIANFU_FIELDS_STR_INFO_END = 26; //  字符串数据结束
    SharedDef.DOUJIANTAI_FIELDS_INT_RANK = 42; //  2 shorts (0:自己的排名, 1:对手的排名)
    SharedDef.DOUJIANTAI_FIELDS_INT_COMBAT_WIN_INFO = 43; //  2 shorts (0:历史最大连胜, 1:当前连胜)
    SharedDef.XIULIANCHANG_FIELDS_INT_RANK = 42; //  敌人斗剑台排行
    SharedDef.XIULIANCHANG_FIELDS_INT_ROBOT_ID = 43; //  敌人机器人序号
    SharedDef.XIULIANCHANG_FIELDS_INT_LEVEL = 44; // 敌人等级 结算经验用
    ...
    // VIP BOSS
    SharedDef.VIP_INSTANCE_FIELD_ID = 42; //  0:VIP副本序号,1:副本难度
    SharedDef.WORLDBOSS_FIELDS_WAIT_TIME = 42; // 报名等待时间戳
    SharedDef.WORLDBOSS_FIELDS_BORN_TIME = 43; // 刷新等待时间戳
    // 试炼塔
    SharedDef.TRIAL_INSTANCE_FIELD_CURSOR = 42; // 当前已经刷了多少波了
    SharedDef.TRIAL_INSTANCE_FIELD_SECTION_ID = 43; // 关卡id
    SharedDef.TRIAL_INSTANCE_FIELD_ORDER = 44; // 刷怪优先级序号(1个byte存一个数据)
    SharedDef.TRIAL_INSTANCE_FIELD_BOSS_REFRESHED = 45; // 是否已经刷怪
    // 家族场景
    SharedDef.FACTION_INSTANCE_FIELD_GUID = 6; //  家族GUID string
    //  桃花迷阵最大刷新次数
    SharedDef.TAOHUA_REFRESH_TIMES = 10;
    // 桃花迷阵地图数据
    SharedDef.TAOHUA_INT_FIELD_REFRESH_TIMES = 42; // 桃花迷阵已经刷新次数
    SharedDef.TAOHUA_INT_FIELD_BOSS_CURR_HP = 43; // BOSS当前血量
    SharedDef.TAOHUA_STR_FIELD_REAL_BOSS_GUID = 5; // BOSS真身GUID
    // 塔的最大层数
    SharedDef.TOWER_MAX_FLOOR = 2;
    // 塔怪每层击杀数量
    SharedDef.TAGUAI_INT_FIELD_FIRST_FLOOR = 42; // 第一层的信息
    // 战斗信息整形字符型枚举
    // 貌似没必要,随便留一个吧
    ...
    SharedDef.MAX_RANK_TYPE = 8;
    // 特殊类型排行榜
    SharedDef.RANK_TYPE_DOUJIANTAI = 9; // 斗剑台排行
    // 单个排行榜最大排名个数
    ...
    // 排行榜
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_FORCE = 4; // 战斗力
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_LEVEL = 6; // 等级
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_MONEY = 7; // 铜钱
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_ONLINE_TIME = 9; // 在线时长
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_TRIAL = 10; // 试炼塔层数
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_SHOW = 11; // 外观列表开始
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_SHOW_END = 15; // 外观结束
    SharedDef.RANK_LIST_CURRENT_OBJ_INT_FIELD_LIKE = 16; // 点赞数
    ...
    // 系统模块
    SharedDef.MODULE_ROLE = 101; // 角色
    SharedDef.MODULE_BAG = 102; // 背包
    SharedDef.MODULE_QUEST = 103; // 任务
    SharedDef.MODULE_CHAT = 104; // 聊天
    SharedDef.MODULE_MAIL = 105; // 邮件
    SharedDef.MODULE_MAP = 106; // 地图
    SharedDef.MODULE_ACHIEVE = 107; // 成就
    SharedDef.MODULE_TITLE = 108; // 称号
    SharedDef.MODULE_STRENGTH = 109; // 我要变强
    SharedDef.MODULE_SPELL = 201; // 技能
    SharedDef.MODULE_DIVINE = 202; // 神兵
    SharedDef.MODULE_FASHION = 203; // 时装
    SharedDef.MODULE_PET = 204; // 宠物
    SharedDef.MODULE_MOUNT = 205; // 坐骑
    SharedDef.MODULE_MIX = 206; // 炼器
    SharedDef.MODULE_MAGIC_WEAPON = 207; // 法宝
    SharedDef.MODULE_CULTIVATION = 208; // 修炼
    SharedDef.MODULE_WING = 209; // 翅膀
    SharedDef.MODULE_MOUNT_GET = 210; // 坐骑
    SharedDef.MODULE_SOCIAL = 301; // 社交
    SharedDef.MODULE_FACTION = 302; // 家族
    SharedDef.MODULE_TEAM = 303; // 队伍
    SharedDef.MODULE_RANK = 304; // 排行榜
    SharedDef.MODULE_COUPLE = 305; // 夫妻
    SharedDef.MODULE_MASTER = 306; // 师徒
    SharedDef.MODULE_MALL = 401; // 商城
    SharedDef.MODULE_VIP = 402; // VIP特权
    SharedDef.MODULE_STALL = 403; // 摊位
    SharedDef.MODULE_AUCTION = 404; // 拍卖行
    SharedDef.MODULE_INSTANCE = 501; // 副本
    SharedDef.MODULE_ACTIVITY = 502; // 活动
    SharedDef.MODULE_ARENA = 503; // 竞技
    SharedDef.MODULE_BOSS = 504; // BOSS
    SharedDef.MODULE_PRACTICE = 505; // 修练场
    SharedDef.MODULE_DAILY_TASKS = 506; // 日常任务
    SharedDef.MODULE_WELFARE = 601; // 福利
    SharedDef.MODULE_RECHARGE_FIRST = 602; // 首充大礼
    SharedDef.MODULE_WEEK = 603; // 登陆大礼
    SharedDef.MODULE_RED_PACKET = 604; // 红包
    SharedDef.MODULE_RECHARGE = 701; // 充值返利
    SharedDef.MODULE_TOTAL_RECHARGE = 702; // 累积充值
    // 系统模块子页签
    SharedDef.MODULE_ROLE_CHAR = 1; // 角色
    SharedDef.MODULE_ROLE_SETTING = 2; // 设置
    SharedDef.MODULE_ROLE_HANG = 3; // 挂机
    SharedDef.MODULE_BAG_ATTR = 1; // 属性
    SharedDef.MODULE_BAG_ITEM = 2; // 背包
    SharedDef.MODULE_QUEST_MAIN = 1; // 主线
    SharedDef.MODULE_QUEST_EXTENSION = 2; // 支线
    SharedDef.MODULE_QUEST_ACTIVITY = 3; // 活动
    SharedDef.MODULE_QUEST_QIYU = 4; // 奇遇
    ...
    SharedDef.MODULE_ACHIEVE_ALL = 1; // 成就
    SharedDef.MODULE_TITLE_ALL = 1; // 称号
    SharedDef.MODULE_STRENGTH_ALL = 1; // 我要变强
    SharedDef.MODULE_SPELL_ALL = 1; // 技能
    SharedDef.MODULE_DIVINE_ALL = 1; // 神兵
    SharedDef.MODULE_FASHION_ALL = 1; // 时装
    SharedDef.MODULE_PET_ALL = 1; // 宠物
    SharedDef.MODULE_MOUNT_UPGRADE = 1; // 进阶
    SharedDef.MODULE_MOUNT_ILLUSION = 2; // 幻化
    SharedDef.MODULE_MIX_STRENGTH = 1; // 强化
    SharedDef.MODULE_MIX_GEM = 2; // 宝石
    SharedDef.MODULE_MIX_WASH = 3; // 洗练
    SharedDef.MODULE_MAGIC_WEAPON_ALL = 1; // 法宝
    SharedDef.MODULE_CULTIVATION_ALL = 1; // 修炼
    SharedDef.MODULE_WING_ALL = 1; // 翅膀
    SharedDef.MODULE_MOUNT_GET_ALL = 1; // 获得
    ...
    SharedDef.MODULE_FACTION_ACTIVITY = 4; // 家族活动
    SharedDef.MODULE_FACTION_TREASURY = 5; // 家族宝库
    SharedDef.MODULE_FACTION_SHOP = 6; // 家族商店
    SharedDef.MODULE_TEAM_ALL = 1; // 队伍
    SharedDef.MODULE_RANK_LOCAL = 1; // 本服排行
    SharedDef.MODULE_RANK_KUAFU = 2; // 跨服排行
    SharedDef.MODULE_COUPLE_ALL = 1; // 夫妻
    SharedDef.MODULE_MASTER_ALL = 1; // 师徒
    ...
    SharedDef.MODULE_STALL_ALL = 1; // 摊位
    SharedDef.MODULE_AUCTION_ALL = 1; // 拍卖行
    SharedDef.MODULE_INSTANCE_PLOT = 1; // 剧情副本
    SharedDef.MODULE_INSTANCE_RES = 2; // 资源副本
    SharedDef.MODULE_INSTANCE_TRIAL = 3; // 试炼塔
    SharedDef.MODULE_INSTANCE_VIP = 4; // VIP副本
    SharedDef.MODULE_INSTANCE_TEAM = 5; // 多人副本
    SharedDef.MODULE_ACTIVITY_DAILY = 1; // 每日活动
    SharedDef.MODULE_ARENA_DOUJIANTAI = 1; // 斗剑台（1V1）
    SharedDef.MODULE_ARENA_XIANMO = 2; // 仙魔道（3V3）
    SharedDef.MODULE_ARENA_XIANFU = 3; // 仙府夺宝
    SharedDef.MODULE_ARENA_5V5 = 4; // 5V5
    SharedDef.MODULE_ARENA_10V10 = 5; // 10V10
    SharedDef.MODULE_ARENA_CONTEST = 6; // 比武大会
    SharedDef.MODULE_BOSS_FIELD_BOSS = 1; // 野外BOSS
    SharedDef.MODULE_BOSS_WORLD_BOSS = 2; // 世界BOSS
    SharedDef.MODULE_PRACTICE_EXP = 1; // 修炼
    SharedDef.MODULE_PRACTICE_ATTACK = 2; // 掠夺
    SharedDef.MODULE_DAILY_TASKS_ALL = 1; // 日常任务
    SharedDef.MODULE_WELFARE_FIRST = 1; // 每日礼包
    SharedDef.MODULE_WELFARE_SIGN = 2; // 每日签到
    SharedDef.MODULE_WELFARE_ONLINE = 3; // 在线礼包
    SharedDef.MODULE_WELFARE_RANK = 4; // 等级有礼
    SharedDef.MODULE_WELFARE_BACK = 5; // 奖励找回
    SharedDef.MODULE_RECHARGE_FIRST_ALL = 1; // 首充大礼
    SharedDef.MODULE_WEEK_ALL = 1; // 七天登陆
    SharedDef.MODULE_RED_PACKET_FACTION = 1; // 家族红包
    SharedDef.MODULE_RED_PACKET_WORLD = 2; // 世界红包
    SharedDef.MODULE_RECHARGE_ALL = 1; // 充值返利
    SharedDef.MODULE_TOTAL_RECHARGE_ALL = 1; // 累积充值
    return SharedDef;
})();
