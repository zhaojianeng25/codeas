...
package modules.hierarchy.h5
{
	...
	{      
		...
		public static function makeMin($rects:Vector.<Rectangle>):Vector.<Rectangle>
		{
			var tw:Number=1;
			var th:Number=1;
			var num:Number=0
			while(true){
				//trace(Math.pow(2,tw),Math.pow(2,th))
				var maxRect:MaxRectsBinPack = new MaxRectsBinPack(Math.pow(2,tw),Math.pow(2,th),false);
				var newRects:Vector.<Rectangle>=new Vector.<Rectangle>;
				for(var i:int = 0; i <$rects.length; i++) {
					newRects.push($rects[i].clone())
				}
				
				maxRect.insert2(newRects, new Vector.<Rectangle>(), MaxRectsBinPack.BestLongSideFit);
				
				if(testIsFinish(maxRect.usedRectangles)){
					
					return maxRect.usedRectangles
					break;
			...
			return null
			function testIsFinish($rects:Vector.<Rectangle>):Boolean
			{
		...
		public function insert2( Rectangles:Vector.<Rectangle>, dst:Vector.<Rectangle>, method:int):void {
			dst.length = 0;
