...
package org.navmesh.findPath
{
	...
	import org.navmesh.geom.Line2D;
	import org.navmesh.geom.PointClassification;
	import org.navmesh.geom.Vector2f;
	import org.navmesh.util.Heap;
	...
	public class NavMesh extends Sprite
	{
		 ...
		 * 
		 */		
		public function searchPath(cellVector:Vector.<Cell>,startPointPx:Point, endPointPx:Point):Array{
			m_CellVector = cellVector;
			openList.setMaxSize(cellVector.length);
			return findPath(startPointPx,endPointPx);
		}
		
		public function getBlock(startPointPx:Point):Boolean{
			var startPos:Vector2f = new Vector2f(startPointPx.x, startPointPx.y);
			return Boolean(findClosestCell(startPos))
		}
...
//				trace("testPtA:" + testPtA)
//				trace("testPtB:" + testPtB)
//				
//				trace("lastPtA:" + lastPtA)
//				trace("lastPtB:" + lastPtB)
//				
//				trace("lastLineA:" + lastLineA)
//				trace("lastLineB:" + lastLineB)
				
				if (!lastPtA.equals(testPtA)) {
					if (lastLineB.classifyPoint(testPtA, EPSILON) == PointClassification.RIGHT_SIDE) {
						//路点
						return new WayPoint(lastBCell, lastPtB);
					} else {
