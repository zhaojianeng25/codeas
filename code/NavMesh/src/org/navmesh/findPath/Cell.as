...
package org.navmesh.findPath
{
	...
	import org.navmesh.geom.LineClassification;
	...
	public class Cell extends Triangle
	{
		...
		public function setLinkIndex(side:int,$index:int):void{
			links[side] = $index;
		}
...
//		}
		...
		public function write(byte:ByteArray):void{
			byte.writeFloat(pointA.x);
			byte.writeFloat(pointA.y);
			byte.writeFloat(pointB.x);
			byte.writeFloat(pointB.y);
			byte.writeFloat(pointC.x);
			byte.writeFloat(pointC.y);
			
			byte.writeShort(_index);
			
			byte.writeShort(_links[0]);
			byte.writeShort(_links[1]);
			byte.writeShort(_links[2]);
		}
