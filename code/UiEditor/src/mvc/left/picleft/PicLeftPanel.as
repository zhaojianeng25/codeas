package mvc.left.picleft
{
	...
	public class PicLeftPanel extends BasePanel
	{
		private var _bg:UIComponent;
		private var _tree:Tree;
		
		public function PicLeftPanel()
		{
			super();
			addList();
		...
		{
			ModuleEventManager.dispatchEvent( new UiSceneEvent(UiSceneEvent.SHOW_PIC_UI_SCENE));
			ModuleEventManager.dispatchEvent( new PicCentenEvent(PicCentenEvent.SHOW_CENTEN));
		...
		protected function onItemClik(event:ListEvent):void
		{
			if(event.itemRenderer){
				...
				$CentenEvent.modelType=$H5UIFileNode.modelType
		...
		{
			_tree.dataProvider = UiData.picItem;
