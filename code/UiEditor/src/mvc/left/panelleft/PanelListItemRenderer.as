package mvc.left.panelleft
{
	...
	public class PanelListItemRenderer extends TreeItemRenderer
	{
		private var _txt:TextInput;	
		
		public function PanelListItemRenderer()
		{
			super();
			addEvents();
			initMenuFile()
		...
		{
			var $selfNode:PanelNodeVo = this.data as PanelNodeVo;
			if($selfNode){
		...
		private function SetLab(value:TreeListData):void{
			...
			var node:PanelNodeVo = value.item as PanelNodeVo;
			if(node.rename){
				if(!_txt){
					_txt = new TextInput;
		...
		protected function onDele(event:Event):void
		{
			...
			if($selfNode){
				...
				var $PanelLeftEvent:PanelLeftEvent=new PanelLeftEvent(PanelLeftEvent.DELE_PANEL_NODE_INFO_VO)
