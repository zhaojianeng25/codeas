package common.utils.ui.collision
{
	...
	public class CollisionUi  extends BaseComponent
	{
		...
		private function addEvents():void
		{
			...
			_tree.addEventListener(Event.COPY,onCopyCollisionVo);
			...
			var itemRestRotation:NativeMenuItem = new NativeMenuItem("调整旋转角度");
			_menuFile.addItem(itemRestRotation);
			
			var itemLast:NativeMenuItem = new NativeMenuItem("添加同上一个");
			//_menuFile.addItem(itemLast);
			...
			itemLast.addEventListener(Event.SELECT,onAddLastItem);
			itemRestRotation.addEventListener(Event.SELECT,onAdditemRestRotation);
			
		}		
		protected function onAdditemRestRotation(event:Event):void
		{
	
		   Alert.show("是否全部将角度Z设置为0","提示",Alert.YES | Alert.NO,null,	function onClose(evt:CloseEvent):void
		   {
			   if(evt.detail == Alert.YES){
				   
				   for(var i:Number=0;i<_collisionItem.length;i++){
					   var $vo:CollisionVo=_collisionItem[i];
					   if($vo.rotationY<=-90){
						   $vo.rotationY=$vo.rotationY+180
					   }
					   if($vo.rotationX<=-90){
						   $vo.rotationX=$vo.rotationX+180
					   }
					   $vo.rotationZ=0
				   }
				   change();
		...
		protected function onAddLastItem(event:Event):void
		{
			if(_collisionItem.length){
				var $copyvo:CollisionVo=_collisionItem[_collisionItem.length-1]
				var $CollisionVo:CollisionVo=new CollisionVo;
				$CollisionVo.name="copy" +_collisionItem.length;
				$CollisionVo.type=$copyvo.type
				$CollisionVo.x=$copyvo.x
				$CollisionVo.y=$copyvo.y
				$CollisionVo.z=$copyvo.z
					
				$CollisionVo.rotationX=$copyvo.rotationX
				$CollisionVo.rotationY=$copyvo.rotationY
				$CollisionVo.rotationZ=$copyvo.rotationZ
					
				$CollisionVo.scale_x=$copyvo.scale_x
				$CollisionVo.scale_y=$copyvo.scale_y
				$CollisionVo.scale_z=$copyvo.scale_z
		...
		protected function onCopyCollisionVo(evt:Event):void
		{
			if(_selectCollisionNode){
				var $copyvo:CollisionVo=_selectCollisionNode.collisionVo
				var $CollisionVo:CollisionVo=new CollisionVo;
				$CollisionVo.name=_selectCollisionNode.name +_collisionItem.length;
				$CollisionVo.type=$copyvo.type
				$CollisionVo.x=$copyvo.x
				$CollisionVo.y=$copyvo.y
				$CollisionVo.z=$copyvo.z
