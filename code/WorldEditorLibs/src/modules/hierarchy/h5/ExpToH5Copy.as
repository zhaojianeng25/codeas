...
//			toBuild()
		...
		private function toBuild():void
		{
			...
			for(var i:uint=0;i<_fileNodeItem.length;i++){
				...
				if($hierarchyFileNode.iModel){
					...
					$tempObj.name=$hierarchyFileNode.name
			...
			$jasobj.sceneCollisionItem=FieldCollisionModel.getInstance().getsceneCollisionItemObject();
		...
		private function returnFun():void{
			...
			this.wirteToBaseScene(astarByte,terrainByte,$Scenebyte)
		...
		private function wirteToBaseScene(astarByte:ByteArray,terrainByte:ByteArray,$Scenebyte:ByteArray):void
		{
			var newUrl1:String = _rootUrl + "map/" + getTerrainName()+"_base.txt"
			MakeResFileList.getInstance().pushUrl(newUrl1)
