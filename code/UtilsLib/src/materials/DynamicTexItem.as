		...
		public function creatTextureByCurve($context3d:Context3D,stage:Stage=null):void{
			...
			if(curveList.length != 0){
				...
				for(var i:int;i<life;i++){
					...
					var $drawColor:Vector3D=new Vector3D(curve.valueVec[0][$drawId],curve.valueVec[1][$drawId],curve.valueVec[2][$drawId],curve.valueVec[3][$drawId])	
					$drawColor.x*=$drawColor.w;
					$drawColor.y*=$drawColor.w;
					$drawColor.z*=$drawColor.w;
					bmp.setPixel32(i,0,argbToHex($drawColor.x,$drawColor.y,$drawColor.z,$drawColor.w));
					//bmp.setPixel32(i,0,argbToHex(1,0,0,1));
				}
			}
