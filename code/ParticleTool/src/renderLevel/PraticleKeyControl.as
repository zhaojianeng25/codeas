package renderLevel
{
	...
	public class PraticleKeyControl extends EventDispatcher
	{
		private static var instance:PraticleKeyControl;
		
		public static function get speedNum():Number
		{
			return _speedNum;
		}
		
		public static function set speedNum(value:Number):void
		{
			_speedNum = value;
		}
		
		public static function getInstance():PraticleKeyControl
		{
			if(!instance)
			{
				instance = new PraticleKeyControl();
			}
			return instance;
		}
		...
		private function makeDownData(event:MouseEvent):void
		{
			...
			if(!Scene_data.selectVec){
				Scene_data.selectVec=new Vector3D()
			}
			...
			else
			{
				if(_isMiddleDown)
				{
					if(event.altKey){
						...
						Scene_data.cam3D.x=_mouseInfo._last_rx+	(_middleMoveVe.x-$v.x)/10;
						Scene_data.cam3D.y=_mouseInfo._last_ry+	(_middleMoveVe.y-$v.y)/10;
						Scene_data.cam3D.z=_mouseInfo._last_rz+	(_middleMoveVe.z-$v.z)/10;
					}
