package mvc.centen.panelcenten
{
	...
	public class PanelCentenProcessor extends Processor
	{
		...
		override protected function receivedModuleEvent($me:ModuleEvent):void 
		{
			switch($me.getClass()) {
				...
				case PanelCentenEvent:
					...
					if($me.action==PanelCentenEvent.EXP_SKILL_MASK_INFO){
						expSkillMaskInfo(PanelCentenEvent($me).panelNodeVo,PanelCentenEvent($me).ctrlKey)
					}
		...
		private function expSkillMaskInfo(panelNodeVo:PanelNodeVo,ctrlKey:Boolean):void
		{
			this._panelCentenView.panelSkillInfoLevel.expMask(panelNodeVo,ctrlKey)
		...
		public function initMenuFile():void{
			...
			item = new NativeMenuItem("添加圆形")
			_menuFile.addItem(item);
			item.addEventListener(Event.SELECT,addCircleBut);
			//CIRCLE
			item = new NativeMenuItem("添加矩形")
			_menuFile.addItem(item);
			item.addEventListener(Event.SELECT,addRectBut);
			
			item = new NativeMenuItem("添加扇形")
			_menuFile.addItem(item);
			item.addEventListener(Event.SELECT,addSectorBut);
			
	
			
		}
		
		protected function addSectorBut(event:Event):void
		{
			...
			PanelModel.getInstance().panelNodeVoAddInfoNode(_panelCentenView.panelNodeVo,pos,PanelRectInfoType.SECTOR)
			var $PanelLeftEvent:PanelLeftEvent=new PanelLeftEvent(PanelLeftEvent.SELECT_PANEL_NODEVO)
			$PanelLeftEvent.panelNodeVo=_panelCentenView.panelNodeVo;
			ModuleEventManager.dispatchEvent($PanelLeftEvent);
		...
		protected function addRectBut(event:Event):void
		{
			var pos:Point=new Point(_panelCentenView.mouseX,_panelCentenView.mouseY)
			pos.x=pos.x-_panelCentenView.PexilX
			pos.y=pos.y-_panelCentenView.PexilY
			...
			PanelModel.getInstance().panelNodeVoAddInfoNode(_panelCentenView.panelNodeVo,pos,PanelRectInfoType.RECTANGLE)
			var $PanelLeftEvent:PanelLeftEvent=new PanelLeftEvent(PanelLeftEvent.SELECT_PANEL_NODEVO)
			$PanelLeftEvent.panelNodeVo=_panelCentenView.panelNodeVo;
			ModuleEventManager.dispatchEvent($PanelLeftEvent);
		}
		
		protected function addCircleBut(event:Event):void
		{
			var pos:Point=new Point(_panelCentenView.mouseX,_panelCentenView.mouseY)
			pos.x=pos.x-_panelCentenView.PexilX
			pos.y=pos.y-_panelCentenView.PexilY
			...
			PanelModel.getInstance().panelNodeVoAddInfoNode(_panelCentenView.panelNodeVo,pos,PanelRectInfoType.CIRCLE)
			var $PanelLeftEvent:PanelLeftEvent=new PanelLeftEvent(PanelLeftEvent.SELECT_PANEL_NODEVO)
			$PanelLeftEvent.panelNodeVo=_panelCentenView.panelNodeVo;
			ModuleEventManager.dispatchEvent($PanelLeftEvent);
			...
			if(!Boolean(event.target is RichEditableText))
			{
				if(event.ctrlKey){
					...
					if(event.keyCode==Keyboard.R){
						...
						evt.panelNodeVo=UiData.panelNodeVo;
						evt.ctrlKey=true;
			...
			if(_panelCentenView.selectArr&&_panelCentenView.selectArr.length){
				...
				Alert.show("是否确定删除？","删除",3,_panelCentenView,function deleteCallBack(event:CloseEvent):void
				{
					if(event.detail == Alert.YES)
					{
						...
						_panelCentenView.selectArr=new Vector.<PanelSkillMaskNode>
					}
		...
		private function deleRectInfoNode(arr:Vector.<PanelSkillMaskNode>):void
		{
			for(var j:uint=0;arr&&j<arr.length;j++)
			{
				var panelRectInfoNode:PanelSkillMaskNode=arr[j]
			...
			$PanelLeftEvent.panelNodeVo=_panelCentenView.panelNodeVo;
			ModuleEventManager.dispatchEvent($PanelLeftEvent);
			arr=new Vector.<PanelSkillMaskNode>
		...
		protected function onStageMouseUP(event:MouseEvent):void
		{
			...
			if(_startSelectNodeInfo){
				for(var i:uint=0;i<_panelCentenView.selectArr.length;i++){
					var $h5UIFileNode:PanelSkillMaskNode=_panelCentenView.selectArr[i];
		...
		private function findSelectNode(event:MouseEvent):void
		{
			if(_panelCentenView.beginLinePoin){  //说明是框选
				var $selectNodeArr:Vector.<PanelSkillMaskNode>=new Vector.<PanelSkillMaskNode>
				...
				for(var i:uint=0;i<_panelCentenView.nodeItem.length;i++){
					var $PanelRectInfoNode:PanelSkillMaskNode=_panelCentenView.nodeItem[i];
		...
		private function delePanelRectInfoNode(panelRectInfoNode:PanelSkillMaskNode):void
		{
			var arr:Vector.<PanelSkillMaskNode>=new Vector.<PanelSkillMaskNode>
		...
		private function selectInfoNode($centenEvent:PanelCentenEvent):void
		{
			...
			for(var i:uint=0;i<_panelCentenView.nodeItem.length;i++){
				...
				for(var j:uint=0;j<_panelCentenView.selectArr.length;j++){
					if(PanelSkillMaskNode(_panelCentenView.nodeItem[i])==_panelCentenView.selectArr[j]){
						has=true;
					}
				}
				PanelSkillMaskNode(_panelCentenView.nodeItem[i]).select=has;
				PanelSkillMaskNode(_panelCentenView.nodeItem[i]).sprite.updata();
			}
			
			
			if(_panelCentenView.selectArr&&_panelCentenView.selectArr.length==1){
				
				if(_panelCentenView.selectArr[0].type==PanelRectInfoType.SECTOR){
		...
		private function showPanelRectInfoNodeButton($PanelRectInfoNode:PanelSkillMaskNode):void
		{
			if($PanelRectInfoNode.type==PanelRectInfoType.SECTOR){
				if(!_panelRectInfoButtonMeshView){
				...
				$PanelRectInfoButtonMesh.panelSkillMaskNode=$PanelRectInfoNode
		...
		protected function panelRectInfoButtonMeshChange(event:Event):void
		{
			...
			$PanelRectInfoButtonMesh.panelSkillMaskNode.sprite.updata();
			
		}
		
		private function showPanelRectInfoNodeGroup(selectArr:Vector.<PanelSkillMaskNode>):void
		{
...
//		PanelRectGroupMesh
		...
		private function showPanelRectInfoNode($PanelRectInfoNode:PanelSkillMaskNode):void
		{
		...
		private function clearSelectNode($node:PanelSkillMaskNode):void
		{ 
			for(var i:uint=0;i<_panelCentenView.selectArr.length;i++){
		...
		private function setPanelNodeVo(panelNodeVo:PanelNodeVo):void
		{
			...
			UiData.panelNodeVo=panelNodeVo;
