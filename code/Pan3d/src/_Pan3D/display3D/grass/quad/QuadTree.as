package _Pan3D.display3D.grass.quad
{
	...
	public class QuadTree
	{
		...
		/**
		 * 添加可视对象到树中 
		 * @param obj 类型为Vector3Ds
		 */		
		public function insertObj( obj:Object ):void{
			var sonNode:Node = searchNodeByPoint(new Point(obj.x,obj.z) , _mainNode );
		...
		private function queryAndAdd(objVec:Vector.<Object>,rect:Rectangle ,  tempNode:Node ,exact:Boolean ):void{
			...
			if(tempNode.hasSon){
				...
				for each(var obj:Object in tempNode.objVec){
					if(exact){
						var sonRect:Rectangle = new Rectangle(obj.x,obj.z,1,1) ;
						
						if(obj.x>=rect.x&&obj.z>=rect.y&&obj.x<(rect.x+rect.width)&&obj.z<(rect.y+rect.height)){
							objVec.push( obj );
						}
...
class Node{
	...
	public function dispose():void
	{
		...
		if(objVec)
		{
			for each( var mc:Vector3D in objVec)
			{
				mc = null ;
			}
