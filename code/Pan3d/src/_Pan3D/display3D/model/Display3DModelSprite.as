package _Pan3D.display3D.model
{
	...
	public class Display3DModelSprite extends ModelHasDepthSprite
	{
		...
		protected function objsFun(obj:ObjData):void
		{
			...
			this.objData=obj;
			if(this.objData){
				for(var i:Number=0;i<this.objData.normals.length/3&&i<100;i++){
					trace(new Vector3D(this.objData.normals[i*3+0],this.objData.normals[i*3+1],this.objData.normals[i*3+2]))
				}
			}
			processTBN();
