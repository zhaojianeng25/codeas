package _Pan3D.utils.editorutils
{
	...
	public class RoleLoadUtils
	{
		private var _ride:Display3DEditorMovie;
		...
		public function RoleLoadUtils(data:Object)
		{
			this._data = data;
			_boneAry = data.bone;
			_meshAry = data.mesh;
			_ride = new Display3DEditorMovie(Scene_data.context3D);
			loadBone();
		...
		public function setRoleData($dis:Display3DEditorMovie,data:Object):void{
			this._data = data;
			_boneAry = data.bone;
			_meshAry = data.mesh;
			_ride = $dis;
		...
		public function getRole():Display3DEditorMovie{
			return _ride;
		}
		...
		public function addLocalAnim(obj:Object):void{
			AnimDataManager.getInstance().addAnim(Scene_data.fileRoot + obj.url,onLocalAnimLoad,obj);
		...
		private function onLocalAnimLoad(ary:Array,obj:Object):void{
			...
			obj.data = ary;
			_ride.addAnimLocal(obj.fileName,ary,obj.bounds,obj.pos);
			
			_loadFlag++;
			if(_loadFlag == _loadlAllNum){
				loadMesh();
				_ride.dispatchEvent(new Event("boneComplete"));
			}
		}
		...
		private function loadMesh():void{
			for(var i:int;i<_meshAry.length;i++){
				...
				MeshDataManager.getInstance().addMesh(Scene_data.fileRoot + obj.url,onMeshFileLoad,obj);
			}
		}
