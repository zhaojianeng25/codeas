package guiji
{
	...
	public class GuijiLevel 
	{
		public var x:int;
		public var y:int;
		public var z:int;

		private var _line3DArrSprite:Line3DArrSprite;
		...
		private var _guijiLizhiVO:Display3DLocusPartilce;
		private static var instance:GuijiLevel;
		private var _choosePointItem:Array
		private var _keyobj : Object = new Object;
		private var _needUpToView:Boolean=false

		public function GuijiLevel()
		{
			super();
			addShaders();
			initData();
		...
		public function get guijiLizhiVO():Display3DLocusPartilce
		{
			return _guijiLizhiVO;
		}

		public function  setGuijiLizhiVO(value:Display3DLocusPartilce,isMustShow:Boolean=false):void
		{
		 ...
		 public function upData():void
		{
			...
			_pos.x=_guijiLizhiVO.x
			_pos.y=_guijiLizhiVO.y
			_pos.z=_guijiLizhiVO.z
		...
		public static  function  Instance():GuijiLevel{
			if(!instance){
				instance = new GuijiLevel();
			}
			return instance;
		}
		...
		private function chooseAllPoint():void
		{
			...
			for(var i:int=0;i<_PointArr.length;i++){	
				...
				tempxyzPosData.x=tempPointObject3D.x*GuijiLevel.scaleNum+_pos.x
				tempxyzPosData.y=tempPointObject3D.y*GuijiLevel.scaleNum+_pos.y
				tempxyzPosData.z=tempPointObject3D.z*GuijiLevel.scaleNum+_pos.z
		...
		public static var  scaleNum :Number=0.1;
		private function stageMouseDown(event:MouseEvent):void
		{
			...
			for(var i:int=0;i<_PointArr.length;i++){	
				var tempPointObject3D:Object3D=_PointArr[i];
				var kitPos:Vector3D=new Vector3D(tempPointObject3D.x*GuijiLevel.scaleNum+_pos.x,tempPointObject3D.y*GuijiLevel.scaleNum+_pos.y,tempPointObject3D.z*GuijiLevel.scaleNum+_pos.z)
				
				var hitit:Boolean=MathHitModel.mathHit3DPoint(kitPos,10);
				if(hitit){
					var tempxyzPosData:XyzPosData=new XyzPosData();
					tempxyzPosData.x=tempPointObject3D.x*GuijiLevel.scaleNum+_pos.x
					tempxyzPosData.y=tempPointObject3D.y*GuijiLevel.scaleNum+_pos.y
		...
		private function isAllKey(pointItem:Array):Boolean
		{
			var isTure:Boolean=true
			var tempXyzPosData:XyzPosData;
			for(var i:uint=0;i<pointItem.length;i++)
			{
				tempXyzPosData=pointItem[i]
				if(tempXyzPosData.id%3!=0){
					//有不是关键的了
					isTure=false
				}
		...
		private function upPointItem(_arr:Array):void
		{
			...
			for(var j:int=0;j<int((_arr.length-1)/3);j++)
			{
				...
				_basePointLine.makeLineMode(a0,a1,1,new Vector3D(0.4,0.4,0.4,1))
				_basePointLine.makeLineMode(a2,a3,1,new Vector3D(0.4,0.4,0.4,1))
			}
		...
		{
			_line3DArrSprite=new Line3DArrSprite(Scene_data.context3D);
	    ...
	    private function puping():void
		{
			...
			for(var i:int=0;i<_PointArr.length;i++){	
				var tempPointObject3D:Object3D=_PointArr[i];
				var tempxyzPosData:XyzPosData=new XyzPosData();
				tempxyzPosData.x=tempPointObject3D.x+this.x
				tempxyzPosData.y=0+this.y
				tempxyzPosData.z=tempPointObject3D.z+this.z
