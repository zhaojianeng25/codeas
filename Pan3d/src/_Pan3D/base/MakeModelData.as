package _Pan3D.base
{
	...
	public class MakeModelData
	{
		public static function makeBoxTampData(hitbox:ObjectHitBox,scale:Number=1):ObjData
		{
		...
		public static function makeJuXinTampData(a:Vector3D=null,b:Vector3D=null):ObjData
		{
			...
			for(id=0;id<n.length;id++){
				_objData.normals.push(n[id])
			}
		...
		public static function makeRectangleData(width:Number,height:Number,offsetX:Number=0.5,offsetY:Number=0.5,isUV:Boolean=false,isU:Boolean=false,isV:Boolean=false,animLine:int=1,animRow:int=1):ObjData{
			var _objData:ObjData=new ObjData;
		...
		public static function makeRectangleMaskData(width:Number,height:Number,offsetX:Number=0.5,offsetY:Number=0.5,isUV:Boolean=false,isU:Boolean=false,isV:Boolean=false,animLine:int=1,animRow:int=1):ObjData{
			var _objData:ObjData=new ObjData;
		...
		public static function makeObjDataToHitBox(objData:ObjData,scale:Number=1,scale_y:Number=0,scale_z:Number=0):ObjectHitBox
		{
			if(scale_y == 0)
			{
				scale_y = scale;
    		...
    		var objectHitBox:ObjectHitBox=new ObjectHitBox;
		 ...
		 * 
		 */
		public static function makeSpriteModelHitBox(objData:ObjData,posMatrix3D:Matrix3D=null):ObjectHitBox
		{
		...
		public static function makeLinkData(width:Number,height:Number,offsetX:Number=0.5,offsetY:Number=0.5,isUV:Boolean=false,isU:Boolean=false,isV:Boolean=false,animLine:int=1,animRow:int=1):ObjData{
			var _objData:ObjData=new ObjData;
		...
		public static function makeDoubleRectangleData(width:Number,height:Number,offsetX:Number=0.5,offsetY:Number=0.5,isUV:Boolean=false,isU:Boolean=false,isV:Boolean=false,animLine:int=1,animRow:int=1,num:int=2,$scale:Number=1):ObjData{
			var _objData:ObjData=new ObjData;
...
//			indexAry.push(0,1,2,0,2,3,4,5,6,4,6,7);
		...
		public static function getCollisionCylinderObjData():ObjData
		{
			var $objData:ObjData=new ObjData;
			...
			$objData.indexs=new Vector.<uint>
			var jindu:uint=12;
			$objData.vertices.push(100,+50,0);
			$objData.vertices.push(100,-50,0);
			for( var i:uint=0;i<=jindu;i++){
				var a:Vector3D=new Vector3D(100,+50,0);
				var b:Vector3D=new Vector3D(100,-50,0);
				var m:Matrix3D=new Matrix3D;
				m.appendRotation(i*(360/jindu),Vector3D.Y_AXIS);
				a=m.transformVector(a)
				b=m.transformVector(b)
				$objData.vertices.push(a.x,a.y,a.z);
				$objData.vertices.push(b.x,b.y,b.z);
				if(i<jindu){
					var $addNum:Number=i*2+2
					$objData.indexs.push(0+$addNum,1+$addNum,3+$addNum)
					$objData.indexs.push(0+$addNum,3+$addNum,2+$addNum)
					
					
					$objData.indexs.push(0,0+$addNum,2+$addNum)
					$objData.indexs.push(1,1+$addNum,3+$addNum)
				}
				
			}
			return $objData
		}
		public static function getCollisionConeObjData():ObjData
		{
			var $objData:ObjData=new ObjData;
			...
			for( var i:uint=0;i<=jindu;i++){
				...
				$objData.vertices.push(b.x,b.y,b.z);
				if(i<jindu){
					var $addNum:Number=i+2
					$objData.indexs.push(0,0+$addNum,1+$addNum)
					$objData.indexs.push(1,1+$addNum,0+$addNum)
				}
				
			}
			return $objData
		}
		public static function getCollisionBallOjbData():ObjData
		{
			var $objData:ObjData=new ObjData;
			var i:uint,j:uint;
			var jindu:Number=36; //经
			var weidu:Number=10 //纬度
			var p:Vector3D;
			...
			for( j=0;j<weidu+1;j++){
				...
				for( i=0;i<jindu;i++){
					p=new Vector3D(0,radiusNum100,0)
					m=new Matrix3D();
					m.appendRotation(j*(180/weidu),Vector3D.Z_AXIS);
					m.appendRotation(i*(360/jindu),Vector3D.Y_AXIS);
			...
			for( j=0;j<weidu;j++){
				for( i=0;i<jindu-1;i++){
					var a:uint=i;
					var b:uint=i+1;
					var c:uint=i+jindu+0
					var d:uint=i+jindu+1
					...
					$objData.indexs.push(a,b,c)
					$objData.indexs.push(b,d,c)
					if(i==0){
						b=0
						a=jindu-1
						d=jindu
		...
		public static function getCollisionBoxObjData():ObjData
		{
			...
			hitbox.endy=$num
			hitbox.endz=$num
			return MakeModelData.makeBoxTampData(hitbox,1);
		}
